...
function createSlide(row, slideIndex, carouselId) {
  ...
  // Handle both single-column (image-only) and two-column (image + content) structures
  if (columns.length === 1) {
    // Single column - image only
    columns[0].classList.add('carousel-hero-slide-image');
    slide.append(columns[0]);
  } else {
    // Multiple columns - image and content
